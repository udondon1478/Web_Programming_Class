...
$sql = "CREATE TABLE `user_tb` (`id` INT AUTO_INCREMENT PRIMARY KEY,`email` VARCHAR(255) NOT NULL UNIQUE,`password` VARCHAR(255) NOT NULL,`name` VARCHAR(255) NOT NULL,`is_admin` BOOLEAN DEFAULT FALSE,`created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,`updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP);";
$dbh->exec($sql); //SQLの実行
...
$dbh->exec($sql);//SQLの実行
echo "管理者ユーザを登録しました<br>";

//一般ユーザの登録
$sql = "INSERT INTO `user_tb` (`email`,`password`,`name`,`is_admin`)
VALUES('x22999xx@aitech.ac.jp','".password_hash('webphp',PASSWORD_DEFAULT)."','ゆう',FALSE);";
$dbh->exec($sql);//SQLの実行
echo "一般ユーザを登録しました<br>";

//チャンネルの登録
$sql = "CREATE TABLE `channel_tb`(`id` INT AUTO_INCREMENT PRIMARY KEY,`name` VARCHAR(255) NOT NULL,`is_public` BOOLEAN DEFAULT TRUE);";
    $dbh->exec($sql);//SQLの実行
